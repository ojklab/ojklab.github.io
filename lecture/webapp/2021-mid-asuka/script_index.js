...
PetiteVue.createApp({
  ...
  //参考：https://qiita.com/bo-san/items/85d734fd07ca3703b16b
  methods: {
    window: (onload = () => {
      setInterval(() => {
        ...
        if (count < 0) {
          count = 0;
          document.getElementById('timer').textContent = 'タイムアウト！';
        }
      }, 1000);
    })
  }
...
// function countDown() {
//   const now = new Date();
//   const limit = now.setSeconds(now.getSeconds() + 5);
...
//   'score'
// ).textContent = `スコア：${storagedData}点でした`;
